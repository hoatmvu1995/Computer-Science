...
int rotate_left(unsigned num, int n)
{
	...
	for(count = 0; count < n; count++) //n represent how many position that num will shift to the left 
	{
			bit = (num & mask) ? 1: 0; //using operator & to get the bit 
			num <<= 1;//shift num 1 position to the left 
			num |= bit;//add the bit to the right of the num by using |
